...
function readController(funcName) {
    ...
    for (let f in controllersCache) {
        ...
        if (idx !== -1) {
            let block = content.substring(idx, idx + 2000); 
...
const outPath = path.join(__dirname, 'api_docs_detailed.md');
fs.writeFileSync(outPath, md);
console.log('Successfully generated api_docs_detailed.md at ' + outPath);
