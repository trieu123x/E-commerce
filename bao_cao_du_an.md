...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| product_id | BIGINT | FK → products | Liên kết sản phẩm |
| image_url | VARCHAR | NOT NULL | URL ảnh sản phẩm |
...
| parent_id | BIGINT | FK → categories | Danh mục cha (nếu có) |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| user_id | INTEGER | NOT NULL, FK → users | Chủ sở hữu giỏ hàng |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| cart_id | BIGINT | NOT NULL, FK → carts | Giỏ hàng chứa item |
| product_id | BIGINT | NOT NULL, FK → products | Sản phẩm trong giỏ |
| quantity | INTEGER | DEFAULT 1, MIN 1 | Số lượng đặt |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| order_id | BIGINT | NOT NULL, FK → orders | Mã đơn hàng |
| product_id | BIGINT | NOT NULL, FK → products | Mã sản phẩm |
| quantity | INTEGER | NOT NULL, MIN 1 | Số lượng mua |
| price | DECIMAL(12,2) | NOT NULL | Giá tại thời điểm mua |
| total | DECIMAL(12,2) | NOT NULL | Thành tiền (quantity * price) |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| order_id | BIGINT | NOT NULL, FK → orders | Mã đơn hàng thanh toán |
| method | ENUM | COD, MOMO, VNPAY | Phương thức thanh toán |
| status | ENUM | PENDING, COMPLETED, FAILED | Trạng thái thanh toán |
| paid_at | TIMESTAMP | DEFAULT NOW | Thời gian thực hiện |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| product_id | BIGINT | NOT NULL, FK → products | Sản phẩm được đánh giá |
| user_id | BIGINT | NOT NULL, FK → users | Người dùng đánh giá |
...
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| user_id | BIGINT | NOT NULL, FK → users | Mã người dùng |
| product_id | BIGINT | NOT NULL, FK → products | Mã sản phẩm yêu thích |
...
| id | BIGINT | PK, AUTO | Khóa chính |
| user_id | BIGINT | NOT NULL, FK → users | Mã người dùng |
| address | VARCHAR(255) | NOT NULL | Địa chỉ chi tiết (số nhà, đường) |
| district | VARCHAR(100) | NOT NULL | Quận/Huyện |
| ward | VARCHAR(100) | NOT NULL | Phường/Xã |
| is_default | BOOLEAN | DEFAULT false | Địa chỉ mặc định |

#### Bảng `product_sales` – Liên kết Sản phẩm & Khuyến mãi

| Cột | Kiểu | Ràng buộc | Mô tả |
|---|---|---|---|
| id | BIGINT | PK, AUTO | Khóa chính |
| product_id | BIGINT | NOT NULL, FK → products | Mã sản phẩm |
| sale_id | BIGINT | NOT NULL, FK → sales | Mã chương trình sale |
...
│   ├── unit/                  # Kiểm thử đơn vị (Controllers)
│   └── integration/           # Kiểm thử tích hợp (Routes)
│
└── src/
...
| GET | `/api/payment/momo/return` | Xử lý kết quả trả về từ MoMo (Redirect) |
| POST | `/api/payment/momo/ipn` | Webhook xử lý trạng thái thanh toán MoMo |
| POST | `/api/payment/vnpay/create` | Tạo URL thanh toán VNPay |
| GET | `/api/payment/vnpay/return` | Xử lý kết quả trả về từ VNPay (Redirect) |
| GET | `/api/payment/vnpay/vnpay_ipn` | Webhook xử lý trạng thái thanh toán VNPay |
...
| 18 | Kiểm thử (Unit test & Integration test) | ✅ Hoàn thành |
| 19 | Tích hợp thanh toán MoMo & VNPay | ✅ Hoàn thành |
...
| Hướng | Mô tả |
|---|---|
| **Thanh toán quốc tế** | Tích hợp Stripe, PayPal cho khách hàng nước ngoài |
