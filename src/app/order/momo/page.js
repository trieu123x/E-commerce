...
export default function MomoOrderPage() {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      const redirectUrl = window.location.origin + "/completed";
      const momoRes = await instance.post("/payment/momo/create", { 
        orderId,
        redirectUrl
      });
      ...
      if (momoRes.data && momoRes.data.payUrl) {
        // Mở MoMo ở tab mới
        window.open(momoRes.data.payUrl, '_blank');

        // Tự gọi IPN để cập nhật trạng thái đơn hàng (workaround cho test)
        try {
          await instance.post('/payment/momo/ipn', {
            orderId: orderId,
            resultCode: 0
          });
        } catch (ipnErr) {
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4 grid grid-cols-1 md:grid-cols-2 gap-10 text-white">
      ...
      <div className="bg-zinc-900 p-6 rounded-sm shadow-lg border border-zinc-800">
        ...
        <div className="mt-8 p-4 bg-black border border-orange-500 rounded-lg shadow-lg">
          <p className="text-center font-semibold text-lg text-orange-400 mb-2">Thanh toán qua Ví MoMo</p>
      ...
      <div className="bg-zinc-900 p-6 rounded-sm shadow-lg border border-zinc-800">
        ...
        <button
          ...
          disabled={loading}
          className="w-full mt-6 bg-gradient-to-r from-pink-500 to-purple-600 text-white py-3 rounded-lg font-semibold hover:scale-105 transition duration-300 shadow-lg hover:shadow-pink-500/40"
        >
          {loading ? "Processing..." : "Create Order & Pay with MoMo"}
        </button>
      </div>
    </div>
  );
